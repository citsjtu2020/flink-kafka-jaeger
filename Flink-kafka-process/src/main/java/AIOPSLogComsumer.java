...
//        "value":"\"POST /hipstershop.CartService/GetCart HTTP/2\" 200 - via_upstream - \"-\" 43 5 1 1 \"-\" \"grpc-go/1.31.0\" \"81cce4ca-f657-9b07-bf7a-c07f0c2d9d55\" \"cartservice:7070\" \"172.20.10.166:7070\" outbound|7070||cartservice.ts.svc.cluster.local 172.20.10.138:59988 10.68.145.247:7070 172.20.10.138:37660 - default"}
        InfluxDBSink<LOGs> influxDBSink = InfluxDBSink.builder()
        .setInfluxDBSchemaSerializer(new LOGSInfluxSerializer())
...
//        DataStream<ObjectNode> streamSource = env.addSource(new MySource());
        ...
        DataStream<LOGs> sourcecsv =
        streamSource.process(new ProcessFunction<ObjectNode,LOGs>() {
            @Override
            public void processElement(ObjectNode value, Context ctx, Collector<LOGs> out) throws Exception {
                if (value.get("value").has("log_name")) {
                    String name = value.get("value").get("log_name").asText().trim();
...
//                String valuevar = value.get("value"個體
                    String valuevar = value.get("value").get("value").asText().trim();
                    LOGs logout = new LOGs();
                    logout.setApi(pod);
