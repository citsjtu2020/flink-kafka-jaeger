...
public final class InfluxDBSinkOptions {
    ...
    public static final ConfigOption<Integer> INFLUXDB_VERSION =
            ConfigOptions.key("sink.influxDB.client.version")
                    .intType()
                    .noDefaultValue()
                    .withDescription("InfluxDB version.");
    ...
    public static InfluxDBClient getInfluxDBClient(final Configuration configuration) {
        ...
        final int influx_version = configuration.getInteger(INFLUXDB_VERSION);
        if (influx_version > 1){
            ...
            return InfluxDBClientFactory.createV1(url,username,password.toCharArray(),bucket,null);
        }
